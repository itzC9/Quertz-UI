...
local themeStyles = {
    ...
    Kali = {
    SchemeColor = Color3.fromRGB(123, 31, 162), -- Deep purple
    Background = Color3.fromRGB(74, 20, 140), -- Dark purple
    Header = Color3.fromRGB(106, 27, 154), -- Medium purple
    TextColor = Color3.fromRGB(255, 182, 193), -- White-pink
    ElementColor = Color3.fromRGB(94, 53, 177) -- Slightly lighter purple
    }
}
local oldTheme = ""
...
function Trinity.MakeLib(TrinxName, themeList)
    ...
    local selectedTab 
    TrinxName = TrinxName or "Library"
    table.insert(Trinity, TrinxName)
    for i,v in pairs(game.CoreGui:GetChildren()) do
        if v:IsA("ScreenGui") and v.Name == TrinxName then
            v:Destroy()
        end
    end
...
minimize.Position = UDim2.new(0.90, 0, 0.137999997, 0) -- Adjusted to the left side
minimize.Size = UDim2.new(0, 21, 0, 21)
minimize.ZIndex = 2
minimize.Image = "rbxassetid://3926305904"
minimize.ImageRectOffset = Vector2.new(284, 4)
...
minimize.MouseButton1Click:Connect(function()
    if not isMinimized then
        ...
        game.TweenService:Create(Main, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
            Size = UDim2.new(Main.Size.X.Scale, Main.Size.X.Offset, 0, MainHeader.Size.Y.Offset), -- Only the title bar height
            Position = UDim2.new(Main.Position.X.Scale, Main.Position.X.Offset, Main.Position.Y.Scale, Main.Position.Y.Offset) -- Keep the same position
        }):Play()
    else
        isMinimized = false
        game.TweenService:Create(minimize, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {
        ...
        wait(0.1)
        game.TweenService:Create(Main, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
            Size = UDim2.new(0, 500, 0, 300), -- Original size (adjust this to your original size)
            Position = UDim2.new(Main.Position.X.Scale, Main.Position.X.Offset, Main.Position.Y.Scale, Main.Position.Y.Offset) -- Keep the same position
        }):Play()
    end
end)
